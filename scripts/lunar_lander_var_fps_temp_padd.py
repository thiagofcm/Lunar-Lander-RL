    ...
    * The angular velocity is in units of 0.4 radians per second. In order to convert to radians per second, the value needs to be multiplied by a factor of 2.5.
    ...
    def step(self, action):
        ...
        if self.steps_since_last_obs >= self.obs_interval:
            ...
            # get_stale_obs() generates a random observation based on 3 templates of possible stale observations.
...
register(
    id="LunarLander_VarFramerate_TempPadd",
    entry_point="scripts.lunar_lander_var_fps_temp_padd:LunarLander_VarFramerate",
