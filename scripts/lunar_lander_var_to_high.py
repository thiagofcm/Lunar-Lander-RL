...
class LunarLander_Var_to_High(LunarLander):
    r"""
    ## Description
    ...
    * The angular velocity is in units of 0.4 radians per second. In order to convert to radians per second, the value needs to be multiplied by a factor of 2.5.
    ...
    def reset(
        ...
        *,
        ...
        self.obs_interval = 1
        self.next_action_step = self.obs_interval
        self.steps_since_last_obs = 0
    ...
    def step(self, action):
        ...
        # Check if an action will be taken this timestep (When ACTION_FPS = SIMULATION_FPS = 50, then action_interval = 1, which means an action is taken every timestep)
        if self.world_step_count % self.action_interval == 0:
            self.current_fps = self.fps_choices[int(action)]
            # Given the current action (FPS), determines the timestep interval for making a new observation
        ...
        if self.world_step_count % self.obs_interval == 0:
            self.last_sampled_obs = self.current_obs.copy() #self.current_obs is initialized on reset function
            self.steps_since_last_obs = 0
            self.episode_frame_count += 1
        else:
        ...
        #     print("ENDED!", terminated, truncated)
...
register(
    id="LunarLander_Var_to_High",
    entry_point="scripts.lunar_lander_var_to_high:LunarLander_Var_to_High",
