...
class LunarLander_FixedFramerate(LunarLander):
    r"""
    ## Description
    ...
    * The angular velocity is in units of 0.4 radians per second. In order to convert to radians per second, the value needs to be multiplied by a factor of 2.5.
    ...
    def step(self, action):
        ...
        truncated = False
        ep_rew = 0
        chosen_fps = self.fps_choices[int(action)]
        obs_interval = self.base_fps // chosen_fps
        max_internal_steps = 500
        internal_step = 0
        sampled_obs = self.current_obs.copy()
        #DEBUG: chosen_fps = 50

        while not (terminated or truncated):    
            
            if internal_step % obs_interval == 0:
                sampled_obs = self.current_obs.copy()

            navigation_action, _ = navigation_model.predict(sampled_obs, deterministic=True)
            obs, reward, terminated, truncated, info = self._physics_step(navigation_action)
            self.current_obs = obs.copy()
            ep_rew+=reward
            internal_step += 1
            #DEBUG: 
            #print("chosen_fps:", chosen_fps)
            #print("obs_interval:", obs_interval)
            #print("navigation_action:", navigation_action)
            #print("obs:", obs)
            #print(f"INTERNAL STEPS: {internal_step}")
            #print(f"EPIOSDE: {self.episode_count}")

            if internal_step >= max_internal_steps:
                truncated = True
                ep_rew -= 100 # penalty for timeout

        self.episode_count+=1
        # DEBUG
        # print(f"EPISODE COUNT: {self.episode_count}")
        # print(f"EPISODE REWARD: {ep_rew}")
        # print(f"PICKED FPS: {chosen_fps}")
        # print("=======================================================================")
        
        info = dict(info)
        info["chosen_fps"] = chosen_fps
        #info["obs_interval"] = obs_interval
        info["ep_reward"] = ep_rew
        #info["internal_steps"] = internal_step
        info["done"] = truncated and not terminated
        return obs, ep_rew, terminated, truncated, info
...
register(
    id="LunarLander_FixedFramerate",
    entry_point="scripts.lunar_lander_fixed_fps:LunarLander_FixedFramerate",
)